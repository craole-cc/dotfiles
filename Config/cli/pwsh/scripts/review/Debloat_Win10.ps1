...
$tweaks = @(
	...
	### Applications
	"ChrisTitusRegistryTweaks",
	"InstallApps_PackageManager",
	"InstallApps_BPO",
	# "InstallApps_NK",
	# "InstallApps_Utilities",
	# "InstallApps_Web",
	# "InstallApps_Creative",
	# "InstallApps_Productivity",
	# "InstallApps_Office",
	# "InstallApps_Programming",
	# "InstallApps_WebStack",
	# "InstallApps_Docker",
	# "InstallApps_Linux_Kali",
	# "InstallApps_Linux_Ubuntu",
	# "InstallApps_NVidiaDriver",
	# "ChangeDefaultApps",
	...
	"InstallContainers", # "UninstallContainers",
	...
	"EnableIEEnhancedSecurity", # "DisableIEEnhancedSecurity",
...
Function ChrisTitusRegistryTweaks {
	...
	If (!(Get-ItemProperty $UpdatesPath DeferQualityUpdatesPeriodInNKCCDays)) { New-ItemProperty -Path $UpdatesPath -Name "DeferQualityUpdatesPeriodInDays" -Type DWord -Value 4 }
...
Function InstallApps_PackageManager {
	Write-Output "Installing Chocolatey"
	Set-ExecutionPolicy Bypass -Scope Process -Force; [System.Net.ServicePointManager]::SecurityProtocol = [System.Net.ServicePointManager]::SecurityProtocol -bor 3072; Invoke-Expression ((New-Object System.Net.WebClient).DownloadString('https://chocolatey.org/install.ps1'))
	choco feature enable -n=allowGlobalConfirmation
	cup choco-cleaner choco-protocol-support chocolatey-core.extension chocolatey-dotnetfx.extension chocolatey-font-helpers.extension chocolatey-windowsupdate.extension -y
...
Function InstallApps_Utilities {
	Write-Output "Utilities"
	cup 7zip.install bitwarden copyq coretemp dotnetcore-runtime.install Everything greenshot hdtune hwinfo.install notepadplusplus.install musicbee potplayer powertoys qalculate qbittorrent qbittorrent-enhanced rainmeter revo-uninstaller systemexplorer wiztree wumt
}
Function InstallApps_Web {
	Write-Output "Web Apps"
	cup anydesk.install git.install google-backup-and-sync google-drive-file-stream GoogleChrome joplin microsoft-edge openvpn speedtest terminals vivaldi.install winscp.install zoom thunderbird birdtray
}
Function InstallApps_Creative {
	Write-Output "Creative Tools"
	cup font-awesome-font fontbase nexusfont.install imagemagick.app InkScape IrfanView irfanviewplugins irfanview-shellextension shotcut.install obs-studio.install
}
Function InstallApps_Productivity {
	Write-Output "Productivity Software"
	cup daxstudio foxitreader PowerBI tabular-editor
}
Function InstallApps_Office {
	Write-Output "Microsoft Office"
	cup Office365Business -params '"/productid:O365HomePremRetail /exclude:""Access Groove Lync OneDrive OneNote Outlook Publisher Teams"""'
}
Function InstallApps_WebStack {
	Write-Output "Web Server Stack"
	cup composer nginx php php-service mariadb.install mongodb.install mongodb-database-tools mongodb-compass nodejs.install redis-64 HeidiSQL
}
Function InstallApps_Docker {
	Write-Output "Docker"
	cup docker.desktop docker-compose lxoffline -y
}
Function InstallApps_Programming {
	Write-Output "Programminig"
	cup autohotkey.install cmdermini firacodenf font-hackgen-nerd python3 nodejs.install terminal-icons.powershell vcredist-all vscode-insiders.install wsl2
}
Function InstallApps_Linux_Kali {
	Write-Output "Linux"
	cup wsl-kalilinux
}
Function InstallApps_Linux_Ubuntu {
	Write-Output "Linux"
	cup wsl-ubuntu-2004
}
Function InstallApps_NVidiaDriver {
	Write-Output "NVidia"
	cup nvidia-display-driverbird
}
Function InstallApps_NK {
	Write-Output "NK"
	cup 7zip.install anydesk.install obs-studio.install revo-uninstaller systemexplorer speedtest vmware-horizon-client wumt zoom
}
Function InstallApps_BPO {
	Write-Output "BPO "
	cup 7zip.install bitwarden notepadplusplus.install revo-uninstaller systemexplorer wumt zoom potplayer powertoys Everything openvpn speedtest google-drive-file-stream GoogleChrome microsoft-edge joplin greenshot obs-studio.install amazon-workspaces anydesk.install vmware-horizon-client thunderbird birdtray
}
...
Function UninstallContainers {
	Write-Output "Uninstalling Containers..."
	If ((Get-WmiObject -Class "Win32_OperatingSystem").Caption -like "*Server*") {
		Uninstall-WindowsFeature -Name "Containers" -WarningAction SilentlyContinue | Out-Null
	}
 Else {
...
Function InstallContainers {
	Write-Output "Installing Containers..."
	If ((Get-WmiObject -Class "Win32_OperatingSystem").Caption -like "*Server*") {
		Install-WindowsFeature -Name "Containers" --WarningAction SilentlyContinue | Out-Null
	}
 Else {
		Enable-WindowsOptionalFeature -Online -FeatureName "Containers" -All -NoRestart -WarningAction SilentlyContinue | Out-Null
	}
