...
Function InstallApps_PackageManager {
	...
	cup choco-cleaner choco-protocol-support chocolatey-core.extension chocolatey-dotnetfx.extension chocolatey-font-helpers.extension chocolatey-vscode.extension chocolatey-windowsupdate.extension
...
Function InstallApps_Utilities {
	...
	cup 7zip.install copyq dotnetcore-runtime.install Everything greenshot hdtune hwinfo.install nordpass powertoys qalculate qbittorrent qbittorrent-enhanced rainmeter revo-uninstaller systemexplorer vcredist-all wiztree wumt
}
Function InstallApps_Web {
	Write-Output "Web Apps"
	cup git.install google-backup-and-sync google-drive-file-stream microsoft-edge-insider-dev openvpn speedtest terminals winscp.install zoom
}
Function InstallApps_Creative {
	Write-Output "Creative Tools"
	cup darktable font-awesome-font fontbase imagemagick.app InkScape IrfanView irfanviewplugins irfanview-shellextension krita
}
Function InstallApps_Productivity {
	Write-Output "Productivity Software"
	cup daxstudio foxitreader PowerBI tabular-editor
}
Function InstallApps_Multimedia {
	Write-Output "Multimedia & Communication Apps"
	cup musicbee obs-studio.install
}
...
Function InstallApps_Programming {
	Write-Output "Programminig & Linux"
	cup cmdermini FiraCode python3 nodejs.install terminal-icons.powershell vscode-insiders.install wsl2 wsl-kalilinux
}
