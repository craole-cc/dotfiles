...
# list
alias gls='git config --list --show-origin'
...
alias gprbom='git fetch origin master && git rebase origin/master && git update-ref refs/heads/master origin/master' # Rebase with latest remote master
...
# # functions
gdv() {
  git diff --ignore-all-space "$@" | "$EDITOR" -R -
}
